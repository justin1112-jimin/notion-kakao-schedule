...
**카카오톡 메시지의 `link.web_url`은 앱에 등록된 "웹 도메인"이어야 함, 안 그러면 엉뚱한 곳으로 연결됨** (2026-09-10 발견/수정): 기존엔 `app/kakao_client.py`의 `send_kakao_memo()`가 "자세히 보기" 버튼 링크를 `https://notion.so`로 하드코딩해뒀는데, 실제 수신한 메시지에서 이 버튼을 누르면 `localhost`로 이동하는 문제가 있었음. 카카오 공식 문서(메시지 REST API)상 링크로 쓰는 도메인은 [카카오 디벨로퍼스] > [앱] > [제품 링크 관리] > [웹 도메인]에 등록돼 있어야 하며, 등록 안 된 도메인은 무시되고 앱에 등록된(아마 초기 로컬 개발 때 넣어뒀던 `localhost`) 도메인으로 대체되는 것으로 추정됨. `app/kakao_client.py`(`send_kakao_memo`가 `link_url` 인자를 받도록 변경) + `app/scheduler.py`(`_app_base_url()` 추가 — Render가 자동 주입하는 `RENDER_EXTERNAL_URL`을 읽어 `/dashboard`로 링크 생성, 없으면 `PUBLIC_BASE_URL` 환경변수, 그마저 없으면 로컬 개발용 `http://localhost:8000`)로 하드코딩 제거. **배포 후 카카오 디벨로퍼스 콘솔에서 `notion-kakao-schedule.onrender.com`을 웹 도메인으로 등록해야 실제로 정상 동작함 — 이 등록은 사람이 콘솔에서 직접 해야 함.**
